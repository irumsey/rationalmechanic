	...
	)
{
	if (!bitData || !initData)
	{
		LUCID_THROW("unable to load texture (invalid arguments)");
	}
	...
	for (size_t j = 0; j < arraySize; j++)
	{
		...
		for (size_t i = 0; i < mipCount; i++)
		{
			...
			{
				LUCID_THROW("unable to load texture (out of bounds)");
			}
	...
	if (!index)
	{
		LUCID_THROW("unable to load texture");
	}
}
	...
	)
{
	...
	GAL_VALIDATE_HRESULT(hr, "unable to load texture");
}

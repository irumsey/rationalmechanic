...
#define   _FIRST (0)
#define    _LAST (graph.nodeCount() - 1)
...
void Machine::reset()
{
	...
	head[0] = head[1] = graph.addNode(Node(0.f, 0.f));
}
...
{
	graph.addInput(Input(_HEAD, _FP0));
}

void Machine::_cagn(Instruction ins)
{
	if (_FLAG1)
		_HEAD = graph.addNode((Node(_FP0, _FP1)));
	else
		graph.getNode(_HEAD) = Node(_FP0, _FP1);
}
